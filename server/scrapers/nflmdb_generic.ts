...
// Parse picks from NFLMDB page — tries JSON first, falls back to player links
function parseNflmdbPicks(html: string): Array<{ pickNumber: number; playerName: string; position?: string }> {
  const decoded = decodeHtmlEntities(html);

  // Strategy 1: Find "selections" JSON array in decoded HTML
  ...
  // Strategy 2: Regex scan for pick+name pairs anywhere in decoded HTML
  ...
  // Strategy 3: Cheerio — player links + surrounding pick number
...
export const scrapeMddbBigBoard = makeNflmdbScraper({
  ...
  url: "https://www.nflmockdraftdatabase.com/mock-drafts/2026/consensus-big-board-2026",
  boardType: "bigboard",
});
